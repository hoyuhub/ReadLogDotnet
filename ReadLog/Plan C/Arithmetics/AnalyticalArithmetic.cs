...
namespace ReadLog.Plan_C.Arithmetics
{
    ...
    public class AnalyticalArithmetic
    {
        ...
        //每家医院每个接口每天的调用次数
        private static List<Count> listDayCount = new List<Count>();

        //每家医院每个接口没秒的调用次数
        private static List<Count> listSecondCount = new List<Count>();

        //每个手机号每分钟调用指定接口的次数
        private static List<Count> listPhoneCount = new List<Count>();
        ...
        /// <returns></returns>
        public  List<LogEntity> GetLogEntitys(List<string> listStr)
        {
        ...
        /// 统计每家医院每个接口每天的调用次数
        /// </summary>
        public void HospPhoneStatistics()
        {
            List<string> listStr = new List<string>();
            //根据问本行公共区的到需要处理的数据，并清空公共区（消费）
            List<LogEntity> list = GetLogEntitys(listStr);
            //执行获取手机短讯接口调用查询方法
